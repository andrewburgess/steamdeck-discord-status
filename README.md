...
and setup as a Non-Steam Game that can be launched from Steam. This will ensure the plugin can detect
when Discord is running.

### Device Name

Your presence reads `on Steam Deck` by default. SteamOS runs on plenty of other hardware now, so
the quick access panel has a **Device Name** field. This can be used to customize the device name.
Clearing the field puts it back to `Steam Deck`. The name is saved on the deck and applies immediately to
whatever is currently running.

### Discord Application ID (advanced)

The bold first line of your status is the _name of the Discord application_ the plugin connects as,
...
plugin's own application. Games that Discord already recognizes are unaffected — those connect as
...
Copy `deck.example.json` to `deck.json` and fill in your deck's hostname and sudo password. SSH uses key
auth, so make sure `ssh deck@<host>` works first.
...
By default a deploy reloads _just this plugin_ through decky rather than restarting
